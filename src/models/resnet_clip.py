...
class Bottleneck1D(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1):
        ...
        if stride > 1 or inplanes != planes * Bottleneck1D.expansion:
...
class AttentionPool1d(nn.Module):
    ...
    def forward(self, x):
        x = x.flatten(start_dim=2).permute(2, 0, 1)  # NCL -> LNC
...
class ModifiedResNet1D(nn.Module):
    """
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        # the 3-layer stem
        self.conv1 = nn.Conv1d(12, width // 2, kernel_size=3, stride=2, padding=1, bias=False)
        ...
        self.attnpool = AttentionPool1d(input_resolution // 32, embed_dim, heads, output_dim)

        self.linear_proj = EcgLinearProjectionHead(5)
...
# Example usage:
# model = ModifiedResNet1D(layers=[3, 4, 6, 3], output_dim=1000, heads=8, input_resolution=224, width=64)
# x = torch.randn(10, 1, 224)  # Batch size of 10, 1 channel, 224 length
# output = model(x)
# print(output.shape)
